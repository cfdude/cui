...
export function NotificationTab({ prefs, machineId, onUpdate }: Props) {
  ...
  return (
    <div className="px-6 pb-6 overflow-y-auto h-full">
      ...
      <div className="py-4">
        <h3 className="text-sm font-semibold text-neutral-900 dark:text-neutral-100 mb-3">Browser Notifications</h3>
        <p className="text-sm text-neutral-600 dark:text-neutral-400 mb-4">
          Native browser notifications work on HTTPS or localhost. Click Enable below and allow notifications when prompted.
        </p>
          ...
          <div className="bg-red-50 dark:bg-red-900/20 border border-red-200 dark:border-red-800 rounded-md p-3 mb-3">
            <div className="flex items-start gap-2">
              ...
              <div className="text-sm text-red-600 dark:text-red-400">
                <p className="font-medium mb-1">Failed to enable browser notifications</p>
                <p>{webPushError}</p>
                
                {/* Helpful troubleshooting tips */}
                {webPushError.includes('HTTPS') && (
                  <ul className="mt-2 space-y-1 text-xs">
                    <li>• Use HTTPS or localhost to enable browser notifications</li>
        ...
        {webPushStatus && !webPushStatus.hasPublicKey && (
          <div className="text-sm text-amber-600 dark:text-amber-400">
            ...
            Server notification keys not configured. Will be auto-configured on first enable.
          </div>
        )}
      </div>
    </div>
